...
public class CamCommandServer extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (args.length == 0) {
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server start <player> <path> [" + String
                .join(":", CamMode.modes.keySet()) + "] [time|ms|s|m|h|d] [loops (-1 -> endless)] " + ChatFormatting.RED + "starts the animation"));
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server interpolation <path> <" + String
                .join(":", CamInterpolation.getMovementNames()) + "> " + ChatFormatting.RED + "set the camera interpolation"));
            sender
                .addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server target <path> <none:self:pos:entity> " + ChatFormatting.RED + "set the camera target"));
            sender
                .addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server add <path> <x> <y> <z> [yaw] [pitch] [roll] [zoom] " + ChatFormatting.RED + "adds a new point to path or creates a new one"));
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server stop <player> " + ChatFormatting.RED + "stops the animation"));
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server list " + ChatFormatting.RED + "lists all saved paths"));
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server remove <name> " + ChatFormatting.RED + "removes the given path"));
            sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server clear " + ChatFormatting.RED + "clears all saved paths"));
        } else {
            ...
            if (subCommand.equals("start")) {
                if (args.length >= 3) {
                    ...
                    if (path != null) {
                        if (args.length >= 4) {
                            ...
                            if (args.length >= 5) {
                                ...
                                if (duration > 0)
                                    ...
                                    sender.addChatMessage(new TextComponentString("Invalid time '" + args[4] + "'!"));
                                    return;
                                }
                                if (args.length >= 6)
                        ...
                        sender.addChatMessage(new TextComponentString("Path '" + args[2] + "' could not be found!"));
                ...
                if (args.length >= 2) {
                    if (CMDCamServer.removePath(sender.getEntityWorld(), args[1]))
                        sender.addChatMessage(new TextComponentString("Path '" + args[2] + "' has been removed!"));
                    else
                        sender.addChatMessage(new TextComponentString("Path '" + args[2] + "' could not be found!"));
                } else
                    sender
                        .addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server remove <name> " + ChatFormatting.RED + "lists all saved paths"));
                ...
                if (args.length >= 3) {
                    ...
                    if (path != null) {
                        ...
                        if (move != null) {
                            ...
                            sender.addChatMessage(new TextComponentString("Interpolation has been set to '" + interpolation + "'!"));
                        } else
                            sender.addChatMessage(new TextComponentString("Interpolation '" + interpolation + "' not found!"));
                    } else
                        sender.addChatMessage(new TextComponentString("Path '" + args[2] + "' could not be found!"));
                } else
                    sender.addChatMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server interpolation <path> <" + String
                        ...
                        if (args.length >= 3) {
                            ...
                            if (target.equals("self")) {
                                ...
                                if (args.length >= 6) {
                                    ...
                                    int j = 3;
                                    CommandBase.CoordinateArg x = parseCoordinate(vec3d.xCoord, args[j++], true);
                                    CommandBase.CoordinateArg y = parseCoordinate(vec3d.yCoord, args[j++], -4096, 4096, false);
                                    CommandBase.CoordinateArg z = parseCoordinate(vec3d.zCoord, args[j++], true);

                                    path.target = new CamTarget.VecTarget(new Vec3d(x.getResult(), y.getResult(), z.getResult()));
                                    sender.addChatMessage(new TextComponentString("Camera will point towards " + x.getResult() + ", " + y.getResult() + ", " + z.getResult()));
                                } else
                                    sender.addChatMessage(new TextComponentString("Invalid position"));
                            } else if (target.equals("entity")) {
                                if (args.length >= 4) {
                                    try {
                                        UUID uuid = UUID.fromString(args[3]);
                                        path.target = new CamTarget.EntityTarget(uuid.toString());
                                        sender.addChatMessage(new TextComponentString("Camera will point towards " + args[3]));
                                    } catch (IllegalArgumentException e) {
                                        sender.addChatMessage(new TextComponentString("Invalid uuid"));
                                    }
                                } else
                                    sender.addChatMessage(new TextComponentString("Missing uuid"));
                            } else
                ...
                sender.addChatMessage(new TextComponentString("Removed all existing paths (in this world)!"));
            }
        }
    }
