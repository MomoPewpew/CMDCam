...
public class CamEventHandlerClient {
            ...
            if (mc.inGameHasFocus) {
                if (CMDCamClient.getCurrentPath() == null) {
                    ...
                    if (KeyHandler.pointKey.isPressed()) {
                        ...
                        mc.thePlayer.addChatMessage(new TextComponentString("Registered " + CMDCamClient.points.size() + ". Point!"));
                    }

                } else {
                    CMDCamClient.tickPath(mc.theWorld, event.renderTickTime);
                }
                ...
                if (KeyHandler.startStop.isPressed()) {
                    if (CMDCamClient.getCurrentPath() != null) {
                        ...
                        try {
                            ...
                            mc.thePlayer.addChatMessage(new TextComponentString(e.getMessage()));
                        }
                }

                while (KeyHandler.clearPoint.isPressed()) {
                    CMDCamClient.points.clear();
                    mc.thePlayer.addChatMessage(new TextComponentString("Cleared all registered points!"));
                }
            }
        }
    ...
    public void worldRender(RenderWorldLastEvent event) {
        ...
        if (CMDCamClient.getCurrentPath() == null && shouldRender && CMDCamClient.points.size() > 0) {
            ...
            for (int i = 0; i < points.length; i++) {
                ...
                GlStateManager.translate(-TileEntityRendererDispatcher.staticPlayerX, -TileEntityRendererDispatcher.staticPlayerY + mc.thePlayer
    ...
    public static void startSelectingTarget(String serverPath) {
        ...
        mc.thePlayer.addChatMessage(new TextComponentString("Please select a target either an entity or a block!"));
    }
